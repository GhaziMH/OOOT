...
namespace SphericalOptimization
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            var angle = new List<double> { 0, 0.15,0.256 };
            ...
            while (LoopSwitch)
            {
                ...
                foreach (var anglei in angle)
                {
                    ...
                    for (int i = 0; i < counter; i++)
                    {
                        ...
                        var angle2 = -2 * angle1;
                        var angle3 = 3 * angle1;
                        ts.Transform(Matrix4x4.CreateRotationY(anglei * Math.PI));
                        ts.Transform(Matrix4x4.CreateRotationX(0.25 * anglei * Math.PI));
                        //Presenter.ShowAndHang(ts);
                        double R = 0.0;
                        ...
                        else
                        {
                            ...
                            for (int iTheta = 0; iTheta < map.GetLength(0); iTheta++)
                                    ...
                                    using (var writer = new StreamWriter(DirData + "newXFile" + i + filename[i].Name + " objfn X" + k + anglei + ".CSV"))
                                    //using (var csv = new CsvWriter(writer, System.Globalization.CultureInfo.InvariantCulture))
                                    {
